...
pub async fn get_courses_for_teacher_db(
    ...
    let courses: Vec<Course> = rows
        ...
        .map(|r| Course {
            ...
            name: r.name.clone(),
            time: r.time.map(NaiveDateTime::from),
        })
        .collect();
    match courses.len() {
...
pub async fn get_course_details_db(
    ...
    if let Ok(row) = row {
        ...
        Err(ServiceError::NotFoundError("Course didn't founded".into()))
    }
}

pub async fn post_new_course_db(pool: &PgPool, new_course: &Course) -> Result<Course, ServiceError> {
    let row = sqlx::query!(
